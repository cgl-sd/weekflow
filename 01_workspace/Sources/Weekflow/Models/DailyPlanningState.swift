...
struct DailyPlanningState: Identifiable, Codable, Hashable {
    ...
    init(
        ...
        self.id = id
        day = LocalDay(date, calendar: calendar)
    ...
    private enum CodingKeys: String, CodingKey {
        case id, day, date, startMinutes, cutoffMinutes
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let id = try container.decodeIfPresent(UUID.self, forKey: .id) ?? UUID()
        let day = try container.decodeIfPresent(LocalDay.self, forKey: .day)
        let date = try container.decodeIfPresent(Date.self, forKey: .date)
            ?? SystemBusinessCalendar.current.date(for: day ?? SystemBusinessCalendar.current.day(containing: .now))
