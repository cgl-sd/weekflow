...
final class WeekflowApplicationMenu: NSObject {
    ...
    func install(addWeeklyGoal: (() -> Void)? = nil) {
        ...
        let mainMenu = NSMenu()
        mainMenu.addItem(topLevelItem(title: "Workflow", submenu: applicationMenu()))
    ...
    private func applicationMenu() -> NSMenu {
        let menu = NSMenu(title: "Workflow")
        menu.addItem(actionItem("关于 Workflow", action: #selector(showAbout)))
        ...
        menu.addItem(.separator())
        menu.addItem(responderItem("隐藏 Workflow", action: #selector(NSApplication.hide(_:)), key: "h"))
        ...
        menu.addItem(.separator())
        menu.addItem(responderItem("退出 Workflow", action: #selector(NSApplication.terminate(_:)), key: "q"))
        return menu
    }
    ...
    private func taskMenu() -> NSMenu {
        ...
        menu.addItem(commandItem("打开所选任务", notification: .weekflowOpenHighlightedTask, key: "\r", modifiers: []))
        menu.addItem(commandItem("开始或暂停计时", notification: .weekflowToggleTimer, key: " ", modifiers: []))
        menu.addItem(commandItem("标记完成或未完成", notification: .weekflowCompleteHighlightedTask, key: "c", modifiers: []))
        menu.addItem(commandItem("进入专注模式", notification: .weekflowFocusHighlightedTask, key: "f", modifiers: []))
        menu.addItem(.separator())
        menu.addItem(commandItem("自动安排", notification: .weekflowAutoScheduleHighlightedTask, key: "x", modifiers: []))
        menu.addItem(commandItem("推迟一天", notification: .weekflowDelayHighlightedTask, key: "d", modifiers: []))
        menu.addItem(commandItem("移入待办箱", notification: .weekflowBacklogHighlightedTask, key: "z", modifiers: []))
        menu.addItem(.separator())
        menu.addItem(commandItem("复制所选任务", notification: .weekflowCopyHighlightedTask))
        menu.addItem(commandItem("剪切所选任务", notification: .weekflowCutHighlightedTask))
        menu.addItem(commandItem("粘贴任务", notification: .weekflowPasteTask))
        menu.addItem(.separator())
        menu.addItem(commandItem("删除所选任务", notification: .weekflowDeleteHighlightedTask))
        return menu
    }
    ...
    @objc private func openSettings() { WeekflowCommand.post(.weekflowOpenSettings) }
    @objc private func createWeeklyGoal() { addWeeklyGoal?() }
    @objc private func createTask() { WeekflowCommand.post(.weekflowAddTask) }
    @objc private func showShortcutHelp() { WeekflowCommand.post(.weekflowShortcutHelp) }

    @objc private func postCommand(_ sender: NSMenuItem) {
        guard let rawValue = sender.representedObject as? String else { return }
        WeekflowCommand.post(Notification.Name(rawValue))
    }
