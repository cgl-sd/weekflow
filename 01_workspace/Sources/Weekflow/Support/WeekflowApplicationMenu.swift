...
final class WeekflowApplicationMenu: NSObject {
    ...
    private static func swizzleMainMenuSetter() {
        ...
        let swizzled = #selector(NSApplication.weekflow_locked_setMainMenu(_:))
        guard let swizzledMethod = class_getInstanceMethod(
            NSApplication.self, swizzled
        ) else { return }
    ...
    private func applicationMenu() -> NSMenu {
        ...
        menu.addItem(responderItem("隐藏其他应用", action: #selector(NSApplication.hideOtherApplications(_:)), key: "h", modifiers: [.command, .option]))
    ...
    private func fileMenu() -> NSMenu {
        ...
        menu.addItem(actionItem("新建周目标", action: #selector(createWeeklyGoal), key: "n", modifiers: [.command, .shift]))
    ...
    private func editMenu() -> NSMenu {
        ...
        menu.addItem(responderItem("重做", action: Selector(("redo:")), key: "z", modifiers: [.command, .shift]))
        menu.addItem(.separator())
    ...
    private func viewMenu() -> NSMenu {
        ...
        menu.addItem(responderItem("进入全屏幕", action: #selector(NSWindow.toggleFullScreen(_:)), key: "f", modifiers: [.command, .control]))
        return menu
    }
    ...
    private func helpMenu() -> NSMenu {
        ...
        menu.addItem(actionItem("键盘快捷键", action: #selector(showShortcutHelp), key: "?", modifiers: [.command]))
        return menu
    }
    ...
    private func commandItem(
        _ title: String, command: AppCommand,
        key: String = "", modifiers: NSEvent.ModifierFlags = [.command]
    ) -> NSMenuItem {
    ...
    @objc private func createWeeklyGoal() { CommandRouter.shared.addWeeklyGoalHandler?() }
