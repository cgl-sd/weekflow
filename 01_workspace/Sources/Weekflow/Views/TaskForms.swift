...
struct QuickTaskFormView: View {
    ...
    private var dateLabel: String {
        ...
        if Calendar.current.isDateInToday(plannedDate) { return "今日" }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
                ...
                .popover(isPresented: $showingStartTimePopover, arrowEdge: .bottom) {
                    ScrollClockTimePopover(
                        ...
                        selectedStartTimeIsEndOfDay = time.map {
                            Calendar.current.startOfDay(for: $0) > Calendar.current.startOfDay(for: plannedDate)
                        } ?? false
                    }
                }
            ...
            Button("") { saveTask(openDetails: true) }
    ...
    var body: some View {
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                Button(action: chooseNextWeek) {
                    quickLabel("W", "下周内", color: .green)
                }
                .buttonStyle(.plain)
                quickDateRow("M", "下月内", days: 30, color: .mint)
                Button { clearSchedule() } label: { quickLabel("S", "以后再做", color: .gray) }
                    .buttonStyle(.plain)
                Button { clearSchedule() } label: { quickLabel("N", "不安排", color: .secondary) }
                    .buttonStyle(.plain)
...
private struct ComposerGoalRow: View {
    ...
    var body: some View {
        Button { selection = goal.id } label: {
            HStack {
