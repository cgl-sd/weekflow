...
struct DateJumpButton: View {
    ...
    private var dateTitle: String {
        ...
        case .threeDayCalendar:
            let end = SystemBusinessCalendar.current.calendar.date(byAdding: .day, value: 2, to: date) ?? date
    ...
    var body: some View {
        HStack(spacing: 5) {
            ...
            if allowsReset && !isToday {
                WeekflowButton(action: resetToday) {
...
struct DateJumpPopover: View {
    ...
    init(
        ...
        let selectedDate = SystemBusinessCalendar.current.calendar.date(
            byAdding: .day,
...
struct PlanningDateJumpPopover: View {
    ...
    init(selectedDate: Binding<Date>) {
        ...
        _displayedMonth = State(
            initialValue: SystemBusinessCalendar.current.calendar.dateInterval(
                of: .month,
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if showsManageChannelAction {
                ...
                WeekflowButton("管理频道") {
                    dismiss()
                    DispatchQueue.main.async { CommandRouter.shared.send(.openChannelSettings) }
                }
                    .buttonStyle(.plain)
                    .font(.system(size: 11))
