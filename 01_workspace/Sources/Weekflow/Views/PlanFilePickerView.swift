...
/// (system panels trigger SwiftUI's private menu reset mechanism).
struct PlanFilePickerView: View {
    ...
    private var pathBar: some View {
        HStack(spacing: 6) {
            WeekflowButton {
                if let parent = currentDirectory.path != "/" ? currentDirectory.deletingLastPathComponent() : nil {
                    loadDirectory(parent)
                }
            } label: {
                Image(systemName: "arrow.up")
                    .font(.system(size: 11, weight: .medium))
                    .foregroundStyle(currentDirectory.path != "/" ? WeekflowPalette.textSecondary : WeekflowPalette.textMuted)
            ...
            WeekflowButton {
                loadDirectory(URL(fileURLWithPath: "/"))
            } label: {
