...
@Test func weeklyGoalCreateAndEditEntriesOpenTheExactTaskDetailPage() throws {
    ...
    #expect(contentSource.contains("case .copy: store.copyHighlightedTask()"))
    #expect(contentSource.contains("case .paste: _ = store.pasteTaskClipboard(on: store.activeDay)"))
    #expect(contentSource.contains("store.deleteTask(goalID: reference.goalID, taskID: reference.taskID)"))
