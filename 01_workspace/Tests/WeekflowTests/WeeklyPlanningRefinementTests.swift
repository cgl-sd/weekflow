...
@Test func weeklyPoolUsesTheGoalOnlyUntilItHasNonemptySubgoals() throws {
    ...
    #expect(store.weeklyPlanningPoolEntries.filter { $0.goal.id == goalID }.count == 1)
    let blankGoal = try #require(store.goals.first(where: { $0.id == goalID }))
    #expect(blankGoal.subgoals.count == 1)
    #expect(blankGoal.displayableSubgoals.isEmpty)
    #expect(GoalService().weeklyPlanningProjection(blankGoal).items.isEmpty)
...
@Test func weeklyRelationshipSwitchLivesInTheTopToolbarInsteadOfTheBoardHeader() throws {
    ...
    #expect(boardSource.contains("WeeklyTaskGrouping.indexBounds(for: entries)"))
    #expect(boardSource.contains("taskCenterY(at: bounds.lowerBound)"))
    #expect(!boardSource.contains("entries.indices.filter { entries[$0].goal.id == displayedGoal.id }"))
...
@Test func weeklyGoalCreateAndEditEntriesOpenTheExactTaskDetailPage() throws {
    ...
    #expect(boardSource.contains("ForEach(displayableSubgoals)"))
...
@Test func weeklyGoalEstimatedTotalSumsItsSubgoalEstimates() throws {
    ...
    #expect(boardSource.contains("displayableSubgoals.reduce(0) { total, subgoal in"))
