...
@Test func stageTenLinkedTaskRemainsBackendOnlyAndWritesActualTimeAndFocusRecord() throws {
    ...
    timer.configureTaskWriter { received, minutes in
        #expect(received == reference)
        taskMinutes += minutes
    }
    timer.configureFocusWriter { mode, minutes, _ in
        focusRecords.append((mode, minutes))
    }
...
@Test func stageTenMenuBarModeSwitchSettlesCurrentSessionBeforeSwitching() throws {
    ...
    timer.configureFocusWriter { _, minutes, _ in recordedMinutes += minutes }

    timer.start(now: .now)
    timer.advance(by: 65)
    timer.stopAndSelect(.study)

    #expect(recordedMinutes == 2)
